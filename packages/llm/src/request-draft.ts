...
export const REQUEST_DRAFT_SYSTEM_PROMPT = [
  ...
  "Определяй количество требований по минимально достаточному циклу действий для решения проблемы. Не схлопывай разные необходимые этапы в одно абстрактное требование. Если из фактов следуют осмотр, устранение неисправности и функциональная проверка после работ, они могут быть отдельными требованиями. Не добавляй осмотр или проверку только ради увеличения количества требований.",
