...
const requestDraftSystemPromptParts = [
  ...
  '<description-normalization explicit-facts="preserve" relations="preserve" referents="preserve" natural-language="normalize" new-facts="forbidden" role-allocation="owner-first" role-reuse="distinct-role-only"> description — свободное описание ситуации, а не готовое значение problem. До нормализации выдели существенные явно переданные факты вместе с отношениями между ними и референтами, затем назначь каждому смыслу владеющую роль. Определяй референт по полному контексту, а не только по ближайшему слову: не перепривязывай признак или отношение к другому объекту и не меняй, какой объект обладает указанным свойством. Нормализуй бытовую лексику естественно и профессионально без обязательного буквального копирования, но не добавляй новых фактов. Не размножай исходную формулировку между title, problem, circumstances, impact, verification и actionPlan. Повтор допустим только при отдельном необходимом назначении роли и должен быть сформулирован соответственно этой роли. </description-normalization>',
  ...
  '<verification-preliminary-ownership verification="unknown-or-assumption" preliminary-check="action" same-unknown-and-action="single-owner" distinct-meanings="both-allowed"> verification хранит самостоятельно значимое неизвестное обстоятельство или предположение, а actionPlan.preliminaryCheck — требуемое действие по его установлению. Если оба поля выражали бы одну неизвестность и одно действие, укажи только preliminaryCheck, а verification: null. Используй оба поля только когда verification сохраняет отдельный необходимый смысл, который не повторяет действие preliminaryCheck.',
