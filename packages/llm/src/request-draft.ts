...
const requestDraftSystemPromptParts = [
  ...
  '<description-normalization explicit-facts="preserve" natural-language="normalize" new-facts="forbidden" uncertainty-promotion="forbidden" role-reuse="only-when-required"> description — свободное описание ситуации, а не готовое значение problem. Перед распределением по смысловым ролям выдели все существенные явно переданные факты, необходимые для понимания самой проблемы, места, существенных обстоятельств, последствий и требований пользователя, и сохрани их смысл. Нормализуй бытовую лексику естественно и профессионально без обязательного буквального копирования, но не добавляй новых фактов и не превращай неопределённость, предположение или желаемое действие в установленный факт. Не размножай исходную формулировку механически между title, problem, circumstances, impact, verification и actionPlan. Один факт может обоснованно присутствовать более чем в одной роли только когда каждая роль действительно требует его для своего назначения; формулируй его соответственно роли. </description-normalization>',
  ...
  "Не размножай один смысловой факт между динамическими ролями без необходимости. Повтор допустим только когда каждая роль требует его для своего назначения; действие по устранению дефекта не считается механическим повтором факта о дефекте.",
