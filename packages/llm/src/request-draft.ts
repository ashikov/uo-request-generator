...
function createRequestDraftSubjectPromptRules(
  ...
  return [
    ...
    "- requestItem не является источником subject или evidence: сгенерированное требование не доказывает техническую причину или предмет проблемы",
...
const requestDraftSystemPromptParts = [
  ...
  "- desiredActions — authoritative требование пользователя; не теряй его и не противоречь ему в descriptive prose, нормализуй только его язык в requestItem без сегментации",
  "- backend сам добавит его в раздел «Прошу:» как один request item; не формируй заголовок раздела и нумерацию",
  ...
  `- requestItem: null без desiredActions; иначе непустая строка до ${primaryRequestDraftLimits.requestItem.max} символов или null при отказе от переформулировки`,
  "Нормализуй requestItem только из исходного desiredActions. Ни description, location, consequences, ни сгенерированные problem, impact, subject или другой текст не являются источником новых желаемых действий, объектов или требований для requestItem.",
  "Сохрани все явно заданные действия и их объекты, отрицания и их область, условия, последовательность, степень определённости и предположительность. Сохрани ограничения «только», «пока», «после», «без», «если», «при необходимости» и равнозначные ограничения.",
  "Убери разговорную оболочку, лишнюю эмоциональность и мат только без изменения смысла. Используй естественную официальную русскую формулировку. Уже нормальную формулировку можно сохранить. Несколько связанных действий или предложений остаются одной строкой requestItem без заголовка «Прошу:», нумерации и переводов строк.",
  "Не добавляй самостоятельные действия, диагностику, осмотр, ремонт или проверку результата, если они не заданы в desiredActions. Не превращай диагностику в ремонт. Не выбирай техническую причину, неисправный узел, способ ремонта, замену, регулировку, исполнителя, сроки, акты, документы или гарантии. Явно названный пользователем узел и действие сохрани как его требование, но не выдавай их за доказанную причину в описательных полях.",
  "Если невозможно уверенно сохранить весь смысл и ограничения в пределах лимита, верни requestItem: null. При противоречивом desiredActions не выбирай одну сторону за пользователя: верни null. Это разрешённый отказ только от переформулировки: backend сохранит полный исходный desiredActions. Не заменяй его общим требованием и не удаляй условия ради сокращения.",
