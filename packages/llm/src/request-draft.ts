...
const requestDraftSystemPromptParts = [
  ...
  '<problem-circumstances-ownership problem="object-place-state-or-defect-observable-signs-duration-repetition-scale" circumstances="independent-only" problem-fact-repetition="forbidden" problem-fact-paraphrase="forbidden" empty-independent-facts="null"> Между problem и circumstances поле problem — исключительный владелец объекта, места, наблюдаемого состояния или дефекта, наблюдаемых признаков, длительности, повторяемости и масштаба. Это не запрещает обоснованно упоминать объект в actionPlan или другой роли по её правилам. В circumstances помещай только самостоятельные входные условия, события, контекст, временные способы эксплуатации и действия, ещё не принадлежащие problem. Не повторяй и не перефразируй там факты problem; сохраняй самостоятельные обстоятельства, а при их отсутствии укажи null.',
