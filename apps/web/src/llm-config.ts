...
export function createLlmGateway(environment: NodeJS.ProcessEnv): LlmGateway {
  ...
  const { LLM_API_KEY, LLM_API_URL, LLM_AUTH_SCHEME, LLM_FOLDER_ID, LLM_MODEL } =
  ...
  const model =
    LLM_MODEL ??
    (LLM_FOLDER_ID === undefined ? undefined : `gpt://${LLM_FOLDER_ID}/yandexgpt/latest`);
