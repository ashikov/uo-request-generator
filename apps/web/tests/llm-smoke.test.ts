...
type GeneratedTestScenario = Extract<TestScenario, { expectedOutcome: "generated" }>;
...
function generatedScenario(overrides: Partial<GeneratedTestScenario> = {}): GeneratedTestScenario {
  return {
    ...
    semanticExpectations: ["Сохранить факт о неработающем освещении."],
    ...overrides,
  };
}
...
describe("runLlmSmokeCheck", () => {
  ...
  it.each([
    ...
    [
      ...
      [
        "Описанная проблема требует проверки.",
        `${COMMON_LEGAL_BASIS_BLOCK} изменён`,
        "Прошу:\n1. Проверить проблему",
      ].join("\n\n"),
    ],
    [
    ...
    [
      "с нормативным блоком после раздела «Прошу:»",
      [
        "Описанная проблема требует проверки.",
        "Прошу:\n1. Проверить проблему",
        COMMON_LEGAL_BASIS_BLOCK,
    ...
    [
      "с произвольным текстом между нормативным блоком и разделом «Прошу:»",
      [
        "Описанная проблема требует проверки.",
        COMMON_LEGAL_BASIS_BLOCK,
    ...
    expect(writeLine).toHaveBeenCalledWith(
      "Сценарий generated-scenario: нарушен формат нормативных оснований",
    );
  });

  it.each([
    [
      "с произвольным текстом после раздела «Прошу:»",
  ...
  it.each([
    ...
    { caseName: "с warning", warnings: ["Уточните неоднозначную формулировку"] },
  ])("не делает warning обязательным для scenario без expectWarning: $caseName", async ({
    warnings,
  }) => {
    const scenario = generatedScenario();
    ...
    expect(writeLine).toHaveBeenCalledWith(
      "Сценарий generated-scenario: автоматические проверки пройдены",
    );
  });
  ...
  it("выводит данные для ручной смысловой проверки generated-сценария", async () => {
    ...
    expect(writeLine).toHaveBeenCalledWith("confirmedProblemSubject: common_area_entrance_door");
