...
`warnings`.
После раздела `Прошу:` приложение обычным кодом добавляет в `body` два заранее
проверенных общих нормативных основания. LLM не получает тексты и ссылки на
нормы, не подбирает законодательство и не формирует индивидуальное юридическое
