...
В `packages/core` также реализованы provider-independent расширенный черновик
первичной заявки и renderer. Они раздельно представляют наблюдаемую проблему,
существенные обстоятельства, известные последствия и риски, предмет проверки,
от одного до пяти требований и предупреждения. Текущий production LLM-контракт
пока возвращает прежние `problem`, `impact` и от одного до трёх `requests`.
Локальный adapter передаёт их новому renderer, оставляя новые необязательные
роли пустыми. Переключение Structured Outputs и system prompt на расширенный
черновик относится к #101.
...
сценария из общего набора fixtures. Сейчас в наборе семь сценариев, поэтому
полный запуск выполняет не более семи запросов:
