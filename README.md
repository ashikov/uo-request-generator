...
выведенное практическое значение или потенциальный риск, предмет проверки, от
одного до пяти обоснованных требований и предупреждения. Успешная ветка строго
проверяется по provider-independent контракту из `packages/core` и
непосредственно передаётся детерминированному renderer. При нескольких
несвязанных проблемах модель возвращает отдельный outcome без частичной заявки.
