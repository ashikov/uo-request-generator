...
умолчанию и включается только точным значением `GENERATION_TRUST_PROXY=true`.
Приложение использует штатный `request.ip` Fastify и не разбирает
`X-Forwarded-For` самостоятельно.

Значение `GENERATION_TRUST_PROXY=true` доверяет forwarding-заголовкам от любого
источника, который может обратиться к backend. Его можно включать только когда
backend недоступен публичному клиенту в обход доверенного reverse proxy, а proxy
перезаписывает `X-Forwarded-For` и `X-Forwarded-Proto` данными фактического
соединения. Сохранение переданных пользователем forwarding-заголовков
небезопасно. Ограниченный allowlist доверенных адресов и сетей будет добавлен
отдельно в issue #61.
...
reverse proxy, до публичного включения необходимо завершить #61 и обеспечить
корректный доверенный `request.ip`.
