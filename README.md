...
скрипт SmartCaptcha расширенным методом, создаёт невидимый виджет и сохраняет
стандартный shield. CAPTCHA запускается только после успешной клиентской
валидации. Один непустой callback-токен создаёт один запрос генерации, после
любого результата виджет сбрасывается, а следующая попытка требует новый токен.
