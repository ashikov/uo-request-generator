...
ежедневного календарного удаления technical logs приложения и применимого
reverse proxy на уровне репозитория. Отдельный production rollout и закрытая
проверка фактического срока ещё не выполнены. Статус #190 остаётся `blocked`.
...
| Structured application logs | Технические события и LLM metadata | Максимум 7 календарных дней. Контракт репозитория подготовлен, фактическое соблюдение не подтверждено | Выполнить отдельное внедрение и закрытую проверку по [контракту technical logs](#technical-logs-утверждённый-контракт-и-незавершённая-фактическая-проверка) |
...
событий приложения и новый runtime-контракт репозитория подготовлены. Оператор утвердил
...
копии app logs и состав proxy logs остаются `UNKNOWN`. Календарный механизм
требует отдельного контролируемого внедрения и закрытой проверки только на чтение.
...
| Поставляемое хранение app logs | `CONFIRMED` для контракта репозитория #282 | `compose.production.yaml` задаёт Docker `syslog`, локальный обязательный сокет и `cache-disabled: "true"`. Выделенные правила rsyslog и logrotate ограничивают рост файла и задают календарное удаление. Прежний `json-file` с `max-size`/`max-file` описывал прежний контракт и не обеспечивал срок. Фактический production runtime ещё не проверен |
...
| Proxy logs | `UNKNOWN` для состава и фактического lifecycle | Последующая закрытая проверка подтвердила, что применимые proxy logs требуют того же календарного срока. Их конфигурация и состав остаются вне репозитория. Результат #181 `not applicable` относится только к отдельному внешнему получателю и не доказывает отсутствия proxy logs |
...
| Уничтожение и подтверждение | `CONFIRMED` для контракта репозитория, `UNKNOWN` для production | Правило #282 ежедневно ротирует выделенный файл проекта и удаляет старые архивы при работающем планировщике. После внедрения оператор проверяет состояние задания и отсутствие доступных записей старше срока. Проверка не заменяет удаления и не является доказательством до фактического применения |
...
7. Контракт #282 направляет stdout/stderr приложения через локальный
   ...
   и запускает единый logrotate при достижении 10 МиБ и ежедневно с `ifempty`,
   `rotate 4` и `maxage 5`.
   Прямое изменение Docker-managed `json-file` files не допускается по [D1].
   Правило действует при успешном ежедневном запуске планировщика. После отдельного
   внедрения в production оператор закрыто проверяет фактический logging driver,
   дополнительные копии, состояние планировщика и отсутствие доступных записей
   старше срока. Контракт репозитория не подтверждает фактический production.
...
| Доступ и жизненный цикл technical logs | `blocked` | Оператор 14 сентября утвердил владельца, минимальные роли, событийный пересмотр, отзыв в тот же день и максимальный срок 7 дней. Изменение приложения #275 и контракт репозитория #282 подготовлены, но внедрение в production, дополнительные копии, proxy logs и фактическое удаление не проверены |
...
| #190 | `[M]` Утвердить доступ и жизненный цикл technical logs | #18, #20, #47, #115, #181 и #182 | Публичный статус на 17 сентября — `blocked`. Доступ, семидневный срок и aliases утверждены. Контракт репозитория #282 подготовлен, но внедрение, дополнительные копии app logs, proxy logs и фактическое удаление не подтверждены |
