...
| Поставляемое хранение app logs | `CONFIRMED` для контракта репозитория #282 | `compose.production.yaml` задаёт Docker `syslog`, локальный обязательный сокет и `cache-disabled: "true"`. Выделенные правила rsyslog и logrotate задают календарное удаление. Прежний `json-file` с `max-size`/`max-file` описывал прежний контракт и не обеспечивал срок. Фактический production runtime ещё не проверен |
...
7. Контракт #282 направляет stdout/stderr приложения через локальный
   ...
   и удаляет записи ежедневным logrotate с `ifempty`, `rotate 4` и `maxage 5`.
