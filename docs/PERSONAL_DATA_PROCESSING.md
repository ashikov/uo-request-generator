...
Решение не опубликовано в GitHub.
...
Решение рабочего диалога зафиксировано локально, но не опубликовано в GitHub.
...
| Будущий журнал согласий #264 | Сейчас `not applicable`. После реализации — поиск доказательства согласия после проверки заявителя | Сейчас `not applicable`; будущий механизм не утверждён | Сейчас `not applicable`; версия не подменяется догадкой | После реализации подтверждённый отзыв отражается как `revoked` с `revocationTimestamp` | Сейчас `not applicable`; будущие сроки, поиск и удаление определяются в #264 |
...
После #264 `consentReceiptId` используется как вспомогательный locator только
