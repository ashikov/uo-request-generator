...
17 сентября 2026 года в #282 подготовлен контракт локального `syslog` и
календарного удаления technical logs приложения и применимого reverse proxy.
Контролируемое внедрение выявило несовместимость запуска скрипта ротации из
rsyslog с Ubuntu AppArmor profile. Проверенный технический вариант оставляет
rsyslog только запись и запускает logrotate отдельным минутным таймером.
На этапе внедрения полный закрытый evidence review по #190 ещё не был завершён.
Итоговое решение оператора от 17 сентября приведено в разделе
[technical logs](#technical-logs-утверждённый-и-проверенный-жизненный-цикл).

18 сентября 2026 года в рамках #191 выполнена первая итерация закрытой
датированной оценки ИСПДн. Повторно сверены нормы: статья 19 по [L1] в
редакции от 26.07.2026, включая пункт 3.1 части 2, введённый [L11]; [L5];
[L6] в редакции от 14.05.2020; [L8] и [L7] без выявленных изменений.
Анонсированный проект нового приказа ФСТЭК взамен [L6] по состоянию проверки
не подписан и не зарегистрирован; его статус отслеживается до финального
решения #191. Оператор подтвердил фактическую границу ИСПДн, масштаб
значительно ниже 100 000 субъектов, отсутствие съёмных машинных носителей и
выполнение базовых требований пункта 13 [L5], а также срок не более 7
календарных дней с автоматическим удалением для provider-level
backup/snapshot-копий technical logs. Классификация ИСПДн по категориям [L5]
не завершена: свободный ввод без автоматического детектора может фактически
содержать специальные категории, поэтому «иные категории» не используются как
подтверждённый вход для уровня защищённости до отдельного обоснования.
Тип актуальных угроз, оценка вреда, уровень защищённости, локализация
собственных баз данных, полнота учёта действий, lifecycle snapshots
persistent volume consent ledger и эффективность мер не завершены. Применение
и способ исполнения статьи 19, части 2, пункта 3.1 к фактическим механизмам
уничтожения отдельно не завершены и остаются blocker до обоснованного
решения; этот вопрос не сводится к типу угроз или отсутствию съёмных
носителей. Подтверждённые пробелы вынесены в #294 и #295. Итог #191 остаётся
`blocked`. Закрытый assessment в репозиторий не помещается.
...
Checkbox state, утверждённые текст и version, а после ответа `consentReceiptId` доступны same-document script. Оператор 15 сентября принял расширение остаточного риска по #193; обновлённый characterization и область решения описаны в [C1_CONSENT.md](C1_CONSENT.md#расширение-принятого-dom-риска-193). 16 сентября оператор также принял integrity-риск изменения consent controls и browser actions; ledger подтверждает получение backend assertion, а не человеческий источник UI-действия. Фактическое вмешательство provider не установлено. Security/privacy review реализации выполнен 15–16 сентября; обязательный повторный review сохраняется только по прежним triggers #193. Буквальное переоткрытие #193 не требуется. Фактическая передача данных provider этим решением не утверждается.
...
| Backend → structured logs | `event`, `requestId`, timestamp, статус, HTTP status, duration, aliases в `provider` и `model`, usage и hash prompt | Пользователь как потенциально определяемое лицо через корреляцию | `Ц4`: диагностика и контроль работы | Контракт репозитория направляет stdout/stderr в выделенный локальный syslog-файл с календарным удалением. Фактический pipeline и семидневный lifecycle подтверждены закрытой проверкой #190. Свободный текст, результат, IP, cookie и CAPTCHA token не входят в контракт. Raw provider/model не читаются при формировании событий и заменяются фиксированными aliases публичных классов конфигурации | [`llm-config.ts`](../apps/web/src/llm-config.ts), [`generation-log.ts`](../apps/web/src/generation-log.ts), [`ARCHITECTURE.md`](ARCHITECTURE.md#структурированное-логирование-генерации) | Основание `Ц4`: статья 6, часть 1, пункт 7, выбрано решением оператора по проекту от 8 сентября 2026 года только для минимальных технических событий диагностики и контроля расходов. Проверенные доступ, срок и aliases зафиксированы в [контракте technical logs](#technical-logs-утверждённый-и-проверенный-жизненный-цикл) |
...
| `Ц4` | Диагностировать запросы и расходы: записать `requestId`, статусы, timestamps, duration и LLM metadata | Оператор и лица с доступом к runtime logs | Максимум 7 календарных дней, включая применимые backup/snapshot-копии; фактический lifecycle подтверждён оператором | Статья 6, часть 1, пункт 7, выбрано решением оператора по проекту от 8 сентября 2026 года только для минимальных технических событий диагностики и контроля расходов. Актуальные решения по `provider`, `model`, `systemPromptHash`, доступу, сроку и уничтожению зафиксированы в [контракте technical logs](#technical-logs-утверждённый-и-проверенный-жизненный-цикл). |
...
| Reverse proxy/runtime participant | Передаёт полный HTTP request и response, может завершать TLS и иметь технический доступ к runtime или logs | Не установлено, управляется ли контур оператором или внешним поставщиком. Частный состав access logs не публикуется; применимые потоки и их семидневный lifecycle подтверждены оператором | Если участвует внешнее лицо, учесть его роль и договор. Поддерживать подтверждённый access и logs contract без раскрытия топологии |
...
| Reverse proxy | Полный HTTP transit и потенциальные access-log metadata | Применимые technical logs входят в максимум 7 календарных дней. Частный состав и фактическое удаление не публикуются | Состав, копии и удаление подтверждены оператором по [контракту technical logs](#technical-logs-утверждённый-и-проверенный-жизненный-цикл) |
| Structured application logs | Технические события и LLM metadata | Максимум 7 календарных дней. Фактический lifecycle подтверждён оператором по #190 | Результат закрытой проверки зафиксирован в [контракту technical logs](#technical-logs-утверждённый-и-проверенный-жизненный-цикл) |
...
### Technical logs: утверждённый и проверенный жизненный цикл

**Публичный статус #190 на 17 сентября 2026 года: `approved`.** Решение оператора
относится к текущему проверенному public-beta runtime. Закрытая проверка
подтвердила metadata-only contract событий приложения, log-safe aliases
`provider` и `model`, прежние ограничения доступа и срока для
`systemPromptHash`, состав применимых app и proxy logs, их общий lifecycle,
доступ, фактическую ротацию и уничтожение в пределах 7 календарных дней.
Дополнительный Docker logging cache отключён; локальные дополнительные и
orphan-копии Docker logs, а также локальные backup-механизмы для technical logs
не обнаружены. Оператор подтвердил, что применимые backup/snapshot-копии также
хранятся не более 7 календарных дней. Частные доказательства не публикуются.
Решение пересматривается при существенном изменении logging/storage topology,
изменении доступа, инциденте или перед #169. Оно не является заявлением о полном
соответствии 152-ФЗ.
...
| Поставляемое хранение app logs | `CONFIRMED` для контракта репозитория #282 | `compose.production.yaml` задаёт Docker `syslog`, локальный обязательный сокет и `cache-disabled: "true"`. rsyslog только пишет в выделенный файл; минутный таймер запускает logrotate с размерным и календарным условиями. Прежний `json-file` с `max-size`/`max-file` не обеспечивал срок |
| Фактический production runtime | `CONFIRMED` для проверенного lifecycle #190 | Закрытая проверка подтвердила фактический pipeline, ротацию, отключение дополнительного Docker cache и отсутствие локальных дополнительных и orphan-копий Docker logs. Детали эксплуатации не публикуются |
| Proxy logs | `CONFIRMED` для применимых потоков | Оператор подтвердил их состав и общий с приложением семидневный lifecycle. Частная конфигурация остаётся вне репозитория. Результат #181 `not applicable` относится только к отдельному внешнему получателю |
...
| Календарный retention | `CONFIRMED` для проверенного runtime | Для app и применимых proxy logs установлен и проверен максимум 7 календарных дней. Оператор подтвердил тот же максимум для применимых backup/snapshot-копий; локальные backup-механизмы для technical logs не обнаружены. Это срок проекта, а не нормативный срок |
| Уничтожение и подтверждение | `CONFIRMED` для проверенного lifecycle | Минутный таймер проверяет размер и ежедневное календарное условие, старые архивы автоматически удаляются при исправном планировщике. Закрытая проверка подтвердила ротацию, продолжение записи и соблюдение календарного максимума для текущего runtime |
| `provider`, `model`, `systemPromptHash` | `CONFIRMED` для решения и application change | Оператор 14 сентября выбрал log-safe aliases для `provider` и `model`. Замена реализована по #275 и проверена локально 15 сентября. `systemPromptHash` сохраняет прежнюю семантику, минимальный доступ и семидневный срок. Фактический runtime проверен отдельно в рамках #190 |
...
7. Контракт #282 направляет stdout/stderr приложения через локальный
   Docker `syslog` в отдельный файл rsyslog, отключает Docker dual-logging cache
   и оставляет rsyslog только запись. Отдельный таймер каждую минуту запускает
   logrotate с `daily`, `maxsize 10M`, `ifempty`, `rotate 4` и `maxage 5`.
   Порог размера проверяется периодически и не является точным лимитом байт.
   `copytruncate` оставляет небольшое окно возможной потери строк при ротации.
   Прямое изменение Docker-managed `json-file` files не допускается по [D1].
   Правило действует при успешной работе планировщика. Контролируемое внедрение
   подтвердило техническую ротацию. Итоговая закрытая проверка #190 также
   подтвердила семидневный lifecycle и отсутствие дополнительных локальных копий.
8. Для proxy logs оператор закрыто инвентаризирует применимые потоки и копии.
   Если отдельный вид logs отключён, фиксируются `not applicable`, проверенная
   конфигурация и дата. Для существующих logs до утверждения состава проверяются
...
фактический runtime, дополнительные копии app logs, состав proxy logs,
...
`requestId`, browser UUID, адрес email и `consentReceiptId` не являются
...
Поиск конкретного доказательства согласия, реализованный в #264, выполняется после
...
| Технические логи приложения | `CONFIRMED`: корреляция событий по `requestId`, без пользовательских текстов и избыточных данных | Индивидуальный механизм приложением не реализован | Индивидуальный механизм не реализован | Запрет генераций не является адресным прекращением Ц4 | Адресное удаление не реализовано. Общий календарный lifecycle подтверждён по #190; действия и подтверждение по индивидуальному обращению остаются предметом #184 |
| Логи proxy | Состав применимых потоков подтверждён по #190; адресный поиск не подтверждён | `UNKNOWN` для индивидуального обращения | `UNKNOWN` для индивидуального обращения | `UNKNOWN` для индивидуального обращения | Общий семидневный lifecycle подтверждён по #190; адресное уничтожение и подтверждение по обращению остаются `UNKNOWN` в #184 |
...
| Журнал согласий #264 | Индивидуальный поиск после проверки заявителя по [контракту ledger](C1_CONSENT.md) | Не обещает остановки внешней обработки | Неизменяемая версия не подменяется догадкой | Подтверждённый отзыв отражается как `revoked` с `revocationTimestamp` | Для consent ledger Ц1 #286 реализован отдельный destruction journal и штатная выгрузка его событий. Оператор составляет отдельный акт и хранит его с применимой выгрузкой три года. Это не подтверждает фактическое уничтожение без конкретного события, выгрузки и акта и не закрывает #184 |
...
независимо. До реализации #264 фиксировались только процедурные действия.
Текущие возможности ledger описаны ниже; исполнение отзыва без подтверждения
не заявляется.
...
Для consent ledger Ц1 узкий технический gap закрыт реализацией #286. При
удалении истёкшей consent-записи событие уничтожения сохраняется в отдельном
destruction journal, а не в удаляемой записи. Штатная выгрузка этого журнала
даёт применимое событие для закрытого подтверждения. Процесс для этой области:
`destruction event` → выгрузка → отдельный акт оператора → отдельное хранение
акта и применимой выгрузки три года с даты уничтожения. Наличие механизма не
утверждает, что уничтожение уже происходило, и не заменяет конкретные событие,
выгрузку и акт.

В этом узком решении `consentReceiptId` служит псевдонимной ссылкой на
конкретную обработку. Он не удостоверяет личность. В процессе #184 как locator
индивидуального обращения он используется только после соразмерной проверки
заявителя. Достаточность такого locator как информации об определённом
физическом лице для приказа № 179 остаётся принятым в #286
интерпретационным риском для beta, а не заявлением о полном соответствии.
...
этого не доказывают. Для областей, кроме реализованного destruction journal
consent ledger Ц1, если подтверждение получить нельзя, фиксируется блокирующий пробел.
...
| --- | --- | --- |
| Адресные действия с proxy logs | Общий состав и lifecycle подтверждены по #190; поиск, действия по обращению и подтверждение индивидуального результата требуют отдельной проверки #184 | Оператор |
...
| Уничтожение собственных данных | Для применимых памяти процесса, limiter, app/proxy logs, журнала и входящих материалов: применимость [L7], фактический механизм и достаточные подтверждения по обращению | Оператор; общий lifecycle логов подтверждён по #190, индивидуальные действия остаются в #184 |
...
Если после проверки другой конкретной области необходимо изменение приложения,
оно не реализуется внутри #184. Для consent ledger Ц1 такой отдельный атомарный
результат уже реализован в #286. Для остальных областей предлагается результат:
...
[контракте technical logs](#technical-logs-утверждённый-и-проверенный-жизненный-цикл).
...
- поддержание проверенного жизненного цикла и повторная проверка по триггерам из
  [контракта technical logs](#technical-logs-утверждённый-и-проверенный-жизненный-цикл)
...
- применимый механизм уничтожения данных. Отдельно установить применение и
  способ исполнения статьи 19, части 2, пункта 3.1 к фактическим механизмам
  уничтожения: нужны ли для уничтожения персональных данных прошедшие оценку
  соответствия средства защиты информации с функцией уничтожения информации.
  Этот вопрос не сводится к типу угроз или наличию съёмных носителей
- утверждённый маршрут взаимодействия с ГосСОПКА при применимом computer incident
- проверка резервных копий и восстановления существующего consent ledger и
  отдельного журнала уничтожения по [контракту #264/#286](C1_CONSENT.md).
  Application-managed backups отсутствуют по решению оператора; наличие либо
  отсутствие копий и snapshots платформы подтверждается отдельно. Возможные
  копии включаются в область доступа, удаления и подтверждения результата
...
browser state, app и proxy logs, provider processing, существующего consent ledger
и журнала уничтожения, а при появлении — будущего audit storage
...
Для подготовки операторской проверки доступен
[обезличенный учебный сценарий #183](INCIDENT_EXERCISE.md).
Он включает документальный учебный прогон без отправки уведомлений и действий
в production. Готовность процедуры этим прогоном не подтверждается.

**Подтверждение оператора от 18 сентября 2026 года:** все роли реагирования
выполняет оператор лично — приём сигнала, первичная квалификация, ограничение
доступа или передачи, расследование, решение и применимые уведомления.
Канал приёма — ранее опубликованный контакт оператора. Отдельная внутренняя
цепочка эскалации при текущем составе не требуется. Маршрут ГосСОПКА — через
формы Роскомнадзора, прямое взаимодействие с НКЦКИ не организовано.
Контакты и incident-маршруты LLM, CAPTCHA и инфраструктурного обработчика
остаются `UNKNOWN`. Общее утверждение получателей #181 не подтверждает эти
возможности. [Безопасная фиксация подтверждения](https://github.com/ashikov/uo-request-generator/issues/183#issuecomment-5727721655)
не означает отправки или приёма уведомлений и не заменяет итоговое одобрение #183.
...
взаимодействие с ГосСОПКА. Для текущего состава оператор 18 сентября подтвердил
маршрут через формы Роскомнадзора, без прямого взаимодействия с НКЦКИ. Документ не
...
| Ограничение роста Docker logs | `already satisfied` | #47 завершена. Фактическое исполнение срока, доступа и удаления подтверждено по [контракту technical logs](#technical-logs-утверждённый-и-проверенный-жизненный-цикл) |
...
| Aliases provider/model в structured logs | `already satisfied` для приложения | Решение оператора от 14 сентября реализовано по #275 и проверено локально 15 сентября: фиксированные aliases публичных классов заменяют raw configuration strings. Фактический logging runtime проверен по #190 |
| Процесс запросов субъектов | `blocked` | 15 сентября минимальный порядок, проверка заявителя и журнал утверждены оператором; синтетический dry run выполнен. Общий lifecycle proxy logs подтверждён по #190, но адресные действия по обращению, действия у получателей и применимое подтверждение уничтожения остаются `UNKNOWN` |
...
| Доступ и жизненный цикл technical logs | `approved` | Итоговое решение оператора от 17 сентября для текущего public-beta runtime: состав app и применимых proxy logs, доступ, дополнительные копии, семидневный срок и уничтожение подтверждены закрытой проверкой |
| Применимые меры защиты ИСПДн | `blocked` | Первая итерация закрытой оценки 18 сентября 2026 года: граница и масштаб подтверждены оператором; классификация категорий по ПП РФ № 1119, тип угроз, оценка вреда, уровень защищённости, учёт действий, lifecycle snapshots volume, применение и способ исполнения пункта 3.1 и эффективность не завершены. Подтверждённые пробелы вынесены в #294 и #295. Атомарный outcome #191 |
...
6. Приняты отдельные минимальные outcomes: процесс запросов субъектов #184,
   ...
   #191 и incident process #183. Итоговый статус #190 от 17 сентября —
   `approved` для текущего проверенного public-beta runtime: применимые потоки,
   копии, доступ, семидневный срок и уничтожение подтверждены оператором.
   Остальные задачи требуют собственных решений.
...
- При появлении audit storage отдельно определить backup, restoration, access
  review и подтверждаемое уничтожение. Проверки уже существующего consent
  ledger и журнала уничтожения #264/#286 не откладываются до этого этапа.
...
| #190 | `[M]` Утвердить доступ и жизненный цикл technical logs | #18, #20, #47, #115, #181 и #182 | Публичный статус по решению оператора от 17 сентября — `approved` для текущего проверенного public-beta runtime. Состав потоков и копий, доступ, срок не более 7 календарных дней и уничтожение подтверждены |
...
| #275 | `[M]` Заменить raw `provider` и `model` в application technical logs на log-safe aliases | Operator decision #190 | Application change реализовано и проверено локально 15 сентября. Итог #190 `approved`; финальная проверка #169 остаётся отдельным решением |
...
Статья 19 по L1, L5, L6, L7 и L8 повторно сверены 18 сентября 2026 года в
рамках #191: редакция L1 от 26.07.2026 подтверждена, пункт 3.1 части 2 статьи 19
действует; L6 действует в редакции от 14.05.2020; изменений L7 и L8 не
выявлено.
