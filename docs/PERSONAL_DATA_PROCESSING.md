...
| `confirmedProblemSubject` | Нужен для выбранного нормативного модуля | Нет. Остаётся в проверенном HTTP/core-вводе и не сериализуется в provider user message | Да, после ответа для deterministic gate с provider `subject` и evidence | Нет | Сохранить закрытым необязательным enum и не добавлять в production logs |
