...
365-дневный срок технического идентификатора браузера и минимизировать область
его передачи.
...
| Технический идентификатор браузера | Случайный UUID в подписанной HTTP-only cookie | Пользователь браузера | `Ц2`: дневной и конкурентный limiter | Cookie действует до 365 дней и имеет `Path=/`, поэтому browser отправляет UUID на все same-origin пути, хотя backend использует его только в `POST /api/generate`. Process-local state логически истекает по правилам limiter, физически удаляется при следующем `acquire()` или перезапуске | [`generation-client-id.ts`](../apps/web/src/generation-client-id.ts), [`generate.ts`](../apps/web/src/routes/generate.ts), [`generation-rate-limiter.ts`](../apps/web/src/generation-rate-limiter.ts) | Основание `Ц2` не выбрано. Обосновать либо минимизировать срок и область передачи. При сужении `Path` удалить legacy-cookie с прежним путём |
...
| `confirmedProblemSubject` | Нужен только для выбранного нормативного модуля | Не входит в provider-owned user message, но определяет system prompt и schema | Нет | Косвенно: выбор влияет на `systemPromptHash` | Сохранить закрытым необязательным enum. Учесть выводимое значение в сроке и доступе к логам |
...
| Browser UUID | Нужен для дневного и конкурентного limiter | Нет | Дневное состояние логически истекает на границе UTC day. Физическое удаление происходит при следующем `acquire()` или restart | Нет | Обосновать либо сократить 365-дневный срок. Ограничить `Path=/` до нужного маршрута либо отдельно обосновать широкий scope. При изменении удалить legacy-cookie с прежним путём |
...
| `Ц2` | Ограничить злоупотребления и расходы: обработать IP, browser UUID, timestamps и counters пользователя | Оператор и reverse proxy/runtime participant | Cookie до 365 дней с `Path=/`. In-memory state логически истекает по окну или UTC day, физически удаляется при следующем `acquire()` или restart | Не выбрано. Не считать автоматически частью согласия на генерацию. **Требует внешней юридической проверки** и решения #187 об обосновании или минимизации срока и scope cookie |
...
| Browser cookie | Подписанный случайный UUID | До 365 дней, текущий `Path=/` | Обосновать либо минимизировать срок и область передачи. При сужении пути удалить legacy-cookie |
...
issue #181. Отдельная issue #193 использует подтверждённые provider facts как входные
данные и должна завершиться проверяемой границей либо документированным
принятием остаточного риска. CSP остаётся defense-in-depth и не считается
изоляцией same-document script от DOM.
...
| Рекомендация о минимизации рядом с формой | Рекомендуемый project control, а не установленный законом отдельный документ или точный UI-текст | Не вводить без необходимости идентифицирующие сведения и данные третьих лиц | Backlog #186 без milestone. Не является release gate |
...
| Рекомендация о минимизации у формы | `post-launch improvement` | #186 остаётся полезным project control без milestone. Отдельная норма о таком UI-тексте не установлена, поэтому это не release gate |
| Срок и область technical browser cookie | `requires application change` | #187 — осознанный release criterion проекта: 365 дней и `Path=/` шире подтверждённой дневной цели. Закон не задаёт точный срок или path |
...
| Browser trust boundary внешнего CAPTCHA script | `requires provider/configuration constraint` | #193 — осознанный security release criterion проекта. Нужны проверяемая граница либо документированное принятие риска. CSP сам по себе не является изоляцией |
...
3. Для внешнего CAPTCHA script закрыта отдельная #193: реализована проверяемая
   browser isolation boundary либо документированно принят остаточный риск
   same-document доступа к DOM. CSP учитывается только как defense-in-depth.
...
7. Как осознанный release criterion проекта срок и область передачи technical
   browser cookie документально связаны с дневной целью либо минимизированы.
   Закон не задаёт точный TTL или `Path`. При изменении `Path=/` удаляется
   legacy-cookie с прежним путём.
...
- Выполнить #186 как рекомендуемую подсказку о минимизации свободного текста.
  Задача не имеет milestone и не блокирует #169 или выпуск `v0.2`.
...
| #187 | `[S]` Минимизировать срок и область передачи browser identifier | Limiter contract #39 | Осознанный release criterion `v0.2`: текущие 365 дней и `Path=/` шире подтверждённой дневной цели. Точные значения закон не задаёт |
...
| #193 | `[M]` Утвердить решение для browser trust boundary внешнего CAPTCHA script | Provider facts #181 используются как входные данные | Осознанный security release criterion `v0.2`: проверяемая граница либо документированное принятие риска без заранее выбранного механизма |
