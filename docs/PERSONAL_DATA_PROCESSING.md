...
конкретную обработку. Он не удостоверяет личность и используется только после
соразмерной проверки заявителя. Достаточность такого locator как информации об
определённом физическом лице для приказа № 179 остаётся принятым в #286
