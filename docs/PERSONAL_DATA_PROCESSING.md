...
Оператор документирует первичные или официальные источники, их даты/версии,
доказанные факты, применимость к фактическому потоку и assumptions. Evidence
получает статус `CONFIRMED`, `NOT_APPLICABLE` или `UNKNOWN`. Существенный
`UNKNOWN` либо `blocked` блокирует положительное решение и закрытие задачи.
Датированный итог оператора — `approved`, `blocked` или `not applicable` —
содержит основание и ответственного. Ни internal assessment, ни этот итог не
