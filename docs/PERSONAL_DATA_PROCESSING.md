...
| `confirmedProblemSubject` | Нужен для выбранного нормативного модуля | Поле не передаётся в user message. Конкретное значение различимо по system prompt и закрытому enum JSON Schema | Да, после ответа для deterministic gate с provider `subject` и evidence | Нет | Сохранить закрытым необязательным enum и не добавлять в production logs |
