...
До выпуска по #264 consent state/version будут добавлены в DOM, а `consentReceiptId` — после долговечной записи. До релиза нужно повторить security/privacy review по существующему trigger #193, обновить characterization test и документировать принятое расширение остаточного риска либо mitigation. Буквальное переоткрытие #193 не требуется. Фактическая передача данных provider этим решением не утверждается.

Данные третьих лиц, специальные категории и биометрические данные намеренно не запрашиваются, но могут появиться в свободном вводе или результате. Детектора или фильтра нет. Согласие пользователя не является согласием третьего лица. Биометрическая идентификация имеет статус `NOT_APPLICABLE` как датированное решение оператора по проекту от 8 сентября 2026 года. Операционное правило для таких данных остаётся downstream blocker.

Решением оператора по проекту от 8 сентября 2026 года на основании текущей evidence-policy по статье 22 зафиксированы `CONFIRMED` применимость и `CONFIRMED` невыполнение обязательного действия. Уведомление не подавалось. Правовой и проектный итог остаётся `blocked`, `accepted_for_beta` не применяется.

Решением оператора по проекту от 8 сентября 2026 года статья 12 остаётся `UNKNOWN` без project exception. #184, #190 и #191 остаются отдельными обязательными результатами.
...
`revoked` и `expired` также не дают допуска.
...
`approved`. Неподанное обязательное уведомление по статье 22, неутверждённые
оператор, контакт, основания, необходимое согласие и другие независимые
blockers не входят в область исключения для статьи 12.
...
остальные release gates. В этой задаче статья 12 не получает ни
`NOT_APPLICABLE`, ни `approved`, ни активного принятия риска: она лишь первый
предполагаемый кандидат на этот порядок, если #182 документирует именно
...
Этого недостаточно для публичного запуска. Решением оператора по проекту от 8 сентября 2026 года установлены оператор, публичный контакт `zayavka@ashikov.ru` и основания `Ц1`–`Ц4`. Для beta по-прежнему обязательны provider-evidence actions из закрытого #181, невыполненное уведомление по статье 22, `UNKNOWN` по статье 12 без project exception, публичная политика и информация о внешней обработке, результаты #183, #184, #190 и #191, невыполненная реализация #264 и controls для технических идентификаторов.
...
| --- | --- | --- | --- | --- |
| `Ц1` | Подготовить заявку: получить свободные поля о пользователе или третьих лицах, передать LLM, сформировать и вернуть результат | Оператор, reverse proxy/runtime participant, LLM provider, пользователь | В приложении только память запроса и текущая страница. Browser/OS могут хранить локальное состояние. Срок и удаление у LLM неизвестны | Статья 6, часть 1, пункт 1, согласие, выбрано решением оператора по проекту от 8 сентября 2026 года. Согласие должно быть отдельным, версионированным, датированным и доказуемым. Consent guard и минимальный durable ledger до LLM относятся к #264 и ещё не реализованы. |
...
| `Ц4` | Диагностировать запросы и расходы: записать `requestId`, статусы, timestamps, duration и LLM metadata | Оператор и лица с доступом к runtime logs | Ротация ограничивает объём и число файлов, но не задаёт срок в днях. Удаление определяется эксплуатацией | Статья 6, часть 1, пункт 7, выбрано решением оператора по проекту от 8 сентября 2026 года только для минимальных технических событий диагностики и контроля расходов. Сохраняются требования #190 к доступу, retention и уничтожению. systemPromptHash может раскрыть наличие подтверждения, но не выбранное значение закрытого enum. Для provider и model сохраняются оба допустимых исхода: log-safe aliases либо отдельно одобренный restricted mode с ограниченными доступом и retention. |
...
**Решение.** Решением оператора по проекту от 8 сентября 2026 года для Ц1 выбрано согласие по статье 6, части 1, пункту 1. Согласие должно быть отдельным, версионированным и доказуемым. Реализация consent guard и минимального durable ledger относится к #264 и ещё не выполнена. #184 остаётся операционным процессом отзыва. До реализации #264 необходимо определить финальную формулировку и версию согласия и evidence retention по официальным источникам.
...
| Механизм отзыва | Нужен, если согласие выбрано основанием | Канал, идентификация, срок, прекращение у обработчиков | #184 остаётся отдельным операционным процессом отзыва. #264 отдельно реализует consent ledger и proposed support-withdrawal locator contract. Реализация #264 ещё не выполнена. |
...
| Уточнение после подтверждения неточности | Уточнить либо обеспечить уточнение в течение 7 рабочих дней и снять блокирование. Применимое уведомление субъекта или представителя — по статье 20, части 3 | [L1, статья 21, часть 2, статья 20, часть 3] |
...
| Локализация и трансграничная передача | `unknown` | Решением оператора по проекту от 8 сентября 2026 года статья 12 остаётся `UNKNOWN`, без project exception. Закрытый #181 предоставляет evidence facts для этой отдельной проверки |
| Уведомление об обработке по статье 22 | `blocked` | Решением оператора по проекту от 8 сентября 2026 года на основании текущей evidence-policy зафиксированы `CONFIRMED` применимость и `CONFIRMED` невыполнение обязательного действия. Уведомление не подавалось. `accepted_for_beta` не применяется |
...
1. Решением оператора по проекту от 8 сентября 2026 года установлены оператор, публичный контакт `zayavka@ashikov.ru` и основания `Ц1`–`Ц4`. Предикаты статьи 18.1, части 1, пунктов 1 и 2 отмечены как `NOT_APPLICABLE` этим датированным решением. Работников, непосредственно обрабатывающих данные, нет. Необходимые и достаточные меры статьи 18.1, включая внутренний контроль или аудит либо доказанную по evidence альтернативу, остаются blocker. Consent ledger и guard по #264 ещё не реализованы.
...
3. Для внешнего CAPTCHA script #193 закрыта документированным принятием
   ...
   review.
4. Решением оператора по проекту от 8 сентября 2026 года на основании текущей evidence-policy статья 22 имеет `CONFIRMED` применимость и `CONFIRMED` невыполнение обязательного действия. Уведомление не подавалось, итог `blocked`, `accepted_for_beta` не применяется. Статья 12 остаётся `UNKNOWN` без project exception, и завершённое применимое действие по transfer не утверждается.
5. Публичная политика и информация о внешней обработке остаются launch blockers. Публичный контакт оператора установлен как `zayavka@ashikov.ru`. Для `Ц1` выбрано согласие, но #264 ещё не реализован. До выпуска по #264 consent state/version будут добавлены в DOM, а `consentReceiptId` — после долговечной записи. До релиза нужно повторить security/privacy review по существующему trigger #193, обновить characterization test и документировать принятое расширение остаточного риска либо mitigation. Буквальное переоткрытие #193 не требуется. Фактическая передача данных provider этим решением не утверждается.
...
1. Какова финальная формулировка и версия согласия для #264?
2. Как подтвердить evidence retention для consent ledger по официальным источникам?
3. Каков операционный outcome для случайных данных третьих лиц и специальных категорий, которые могут попасть в свободный ввод или результат при отсутствии детектора или фильтра?
4. Какой маршрут transfer и уведомлений применяется при сохраняющемся `UNKNOWN` по статье 12 без project exception?
5. Каковы сроки будущего audit storage и его backup?
6. Какие идентификаторы и проверка заявителя нужны для процесса #184?
...
9. Как выполнить подтверждённое обязательное уведомление по статье 22 до обработки?
...
12. Какой набор мер статьи 18.1 является необходимым и достаточным, включая внутренний контроль или аудит либо evidence-based alternative, если ознакомление работников `NOT_APPLICABLE` из-за отсутствия работников, непосредственно обрабатывающих данные?
