...
**Решение.** В рамках #24 не добавлять чекбокс согласия. Сначала оператор по evidence
...
| --- | --- | --- | --- |
| Оператор публичного экземпляра | Оператором является лицо, фактически определяющее цели, состав и действия обработки [L1, статья 3] | Репозиторий не устанавливает это физическое или юридическое лицо и его реквизиты | Вне репозитория определить оператора и контакт. Если оператор является юридическим лицом, назначить ответственного по статье 18.1, части 1. **Требует решения оператора по evidence** |
...
| Оператор и основания целей | `requires operator decision` | Факты отсутствуют в репозитории |
| Поручения внешним обработчикам | `requires operator decision` | Роль и договор зависят от каждого provider |
| Локализация и трансграничная передача | `requires operator decision` | Нужны endpoint-specific факты и уведомления |
| Уведомление об обработке по статье 22 | `requires operational procedure` | Для описанного автоматизированного public flow действует default уведомления до обработки. Исключение требует конкретного доказательства пункта 7, 8 или 9 части 2 и решения оператора по evidence |
...
1. Установлены оператор публичного экземпляра, контакт и основания каждой
   ...
   контроль или аудит и применимое ознакомление работников либо проверенное
   оператором по evidence обоснование другого достаточного набора.
...
4. Уведомление Роскомнадзора об обработке выполнено до начала описанного
   ...
   пункта 7, 8 или 9, и решении оператора по evidence. Отдельное уведомление о
