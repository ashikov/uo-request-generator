...
Полный закрытый evidence review фактического срока и доступа по #190 ещё не
завершён; статус остаётся `blocked`.
...
| Reverse proxy | Полный HTTP transit и потенциальные access-log metadata | Применимые technical logs входят в максимум 7 календарных дней. Частный состав и фактическое удаление не публикуются | Подтвердить состав, копии и удаление по [контракту technical logs](#technical-logs-утверждённый-контракт-и-незавершённая-фактическая-проверка) |
| Structured application logs | Технические события и LLM metadata | Максимум 7 календарных дней. Техническая ротация проверена после внедрения; полный срок ещё требует evidence review #190 | Завершить закрытую проверку по [контракту technical logs](#technical-logs-утверждённый-контракт-и-незавершённая-фактическая-проверка) |
...
**Публичный статус #190 на 17 сентября 2026 года: `blocked`.** Контракт
событий приложения и runtime-контракт репозитория подготовлены. Оператор утвердил
...
же границах доступа и срока. Контролируемое внедрение подтвердило отдельные
технические инварианты app и proxy logging, но полный закрытый evidence review
состава копий, доступа и фактического календарного удаления ещё не завершён.
Изменение приложения для aliases реализовано в рамках #275 и проверено локально
15 сентября. По evidence-policy такой результат нельзя считать `approved`.
...
| Фактический production runtime | `CONFIRMED` для технического pipeline, полный review #190 открыт | Контролируемое внедрение проверило исправленный технический вариант. Детали эксплуатационной проверки остаются закрытыми. Полный evidence review срока и доступа ещё не завершён |
| Proxy logs | `CONFIRMED` для технического pipeline, полный состав требует review #190 | Применимые proxy logs входят в тот же публичный syslog-контракт. Состав и частная конфигурация остаются вне репозитория. Результат #181 `not applicable` относится только к отдельному внешнему получателю |
...
| Уничтожение и подтверждение | `CONFIRMED` для технической ротации, полный срок требует review #190 | Минутный таймер проверяет размер и ежедневное календарное условие, старые архивы удаляются при исправном планировщике. Контролируемое внедрение подтвердило ротацию после превышения порога и успешную запись после неё. Отсутствие доступных записей старше срока и полный lifecycle ещё требуют отдельного закрытого evidence review |
...
7. Контракт #282 направляет stdout/stderr приложения через локальный
   ...
   подтвердило техническую ротацию, но полный закрытый review отсутствия
   доступных записей старше срока и всего lifecycle по #190 ещё не завершён.
...
| Доступ и жизненный цикл technical logs | `blocked` | Оператор 14 сентября утвердил владельца, минимальные роли, событийный пересмотр, отзыв в тот же день и максимальный срок 7 дней. После контролируемого внедрения проверены отдельные технические инварианты app и proxy logs; полный evidence review доступа, копий и фактического удаления остаётся открытым |
...
6. Приняты отдельные минимальные outcomes: процесс запросов субъектов #184,
   ...
   #191 и incident process #183. На 17 сентября #190 имеет статус `blocked`.
   Доступ, максимальный семидневный срок и aliases утверждены, контракт
   репозитория #282 подготовлен. Внедрение в production, дополнительные копии,
   proxy logs и фактическое удаление не проверены.
...
| #190 | `[M]` Утвердить доступ и жизненный цикл technical logs | #18, #20, #47, #115, #181 и #182 | Публичный статус на 17 сентября — `blocked`. Доступ, семидневный срок и aliases утверждены. Техническая ротация app и proxy logs проверена после контролируемого внедрения; полный evidence review доступа, копий и фактического удаления ещё не завершён |
