...
проверяет состав proxy logs и отсутствие дополнительных копий. Проверенный
технический pipeline не заменяет полного evidence review: статус #190 остаётся
`blocked` до отдельного решения оператора.
